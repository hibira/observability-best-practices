...
これらは効率的な呼び出しですが、前述の不適切な呼び出しだったらどうでしょうか? クラスター内の合計 50,000 個の Pod に対して、1,000 ノードそれぞれ上の上記の DaemonSet の 1 つが更新を要求していると想像してみてください。次のセクションでは、この無制限のリスト呼び出しのアイデアについて検討します。
